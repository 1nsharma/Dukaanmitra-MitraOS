...
const Layout: React.FC<LayoutProps> = ({ children, activeView, setView, isPremium = true, isAdmin = false }) => {
  ...
  return (
    <div className={`flex h-[100dvh] w-full overflow-hidden font-sans transition-colors duration-1000 ${isAdmin ? 'bg-[#0a0c10]' : 'bg-slate-50'}`}>
      {/* Sidebar - Desktop */}
      <aside className={`hidden lg:flex w-72 flex-col shrink-0 border-r transition-all duration-1000 relative overflow-hidden ${isAdmin ? 'bg-slate-900 border-white/5 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
        {/* Subtle Watermark */}
        <div className="absolute bottom-[-10%] right-[-10%] text-[12rem] font-black italic opacity-[0.02] pointer-events-none select-none">
          MUNIM
        </div>

        <div className="p-10 border-b border-white/10 cursor-pointer group relative z-10" onClick={() => setView('landing')}>
          <motion.h1 
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            className={`text-3xl font-black tracking-tighter italic transition-all duration-500 ${isAdmin ? 'text-white' : 'text-indigo-600'}`}
          >
            DukaanMitra
          </motion.h1>
        ...
        <nav className="flex-1 p-6 space-y-1 overflow-y-auto no-scrollbar relative z-10">
          ...
          {navItems.map((item, i) => (
            ...
            >
              <div className="flex items-center space-x-4">
                <span className={`text-xl transition-transform group-hover:scale-125 ${activeView === item.id ? 'scale-110' : ''}`}>{item.icon}</span>
        ...
        <div className={`p-6 border-t relative z-10 ${isAdmin ? 'border-white/10 bg-black/20' : 'border-slate-100 bg-slate-50/50'}`}>
          <div className="mb-4 px-4 py-3 bg-white/5 rounded-2xl border border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
              <span className="text-[8px] font-black uppercase tracking-widest text-slate-500">System Live</span>
            </div>
            <span className="text-[8px] font-black uppercase tracking-widest text-slate-600">v2.0.0</span>
          </div>
          <button 
